...
class BayesianNeuralNetwork(VarDictParser):
	...
	MODEL_DETAILS = {'burnin': 10**3, 'thinning': 10, 'num_epochs': 5 * 10**4, 'num_draws': 10**4, 'learning_rate': 0.01,
					 'num_layers': 3, 'hidden_shape': 4,
	...
	def __init__(self, var_dicts, observed_params, observed_losses, batch_size, backend = 'pymc3', model_details = None):
		...
		if self.batch_size == 1:
			...
			self.lambda_values = np.linspace(-0.25, 0.75, self.batch_size)
	...
	def build_penalties(self):
		...
		trace_mus_float = self.network.trace['loc'][self.model_details['burnin']::self.model_details['thinning']]
		trace_sds_float = self.network.trace['scale'][self.model_details['burnin']::self.model_details['thinning']]
		trace_mus_int   = self.network.trace['loc'][self.model_details['burnin']::self.model_details['thinning']]
		trace_sds_int   = self.network.trace['int_scale'][self.model_details['burnin']::self.model_details['thinning']]
		trace_cat_probs = [self.network.trace['dirich_%d' % counter][self.model_details['burnin']::self.model_details['thinning']] for counter in range(self.network.num_cats)]
		...
		self.dist_evaluator = DistEvaluator(mus_float, sds_float, mus_int, sds_int, cat_probs, self.observed_losses, self.var_p_type_indicators)
