...
#		self.network_output = self.network_input.copy()
	...
	def _create_model(self):
		...
		with pm.Model() as self.model:
			...
			tau        = pm.Gamma('tau', 10**4 * self.num_obs**2, 1., shape = (self.num_obs, self.network_input.shape[1]))
			...
			# learn the integers
			self.out_ints = DiscreteLaplace('out_ints', loc = self.loc[:, self.ints], scale = self.scale[:, self.ints], observed = self.network_output[:, self._ints])
...
#			self.normal_broadened = pm.Normal('normal_broadened', mu = self.loc, tau = self.tau)
			...
			self.alpha = pm.Deterministic('alpha', self._loc * self.tau + 1.)
...
#					out_dirichlet = pm.Dirichlet('dirich_%d' % dist_counter, a = self.alpha[:, begin : end], shape = (self.num_obs, int(end - begin)), observed = self.network_output[:, begin:end])
					out_dirichlet = pm.Dirichlet('dirich_%d' % dist_counter, a = self.alpha[:, begin : end], shape = (self.num_obs, int(end - begin)) )

#					normal_broadened = pm.Normal('norm_broad_%d' % dist_counter, self.loc[:, begin:end], tau = self.tau[:, begin:end], shape = (self.num_obs, int(end - begin)))
#					out_dirichlet = pm.Deterministic('dirich_%d' % dist_counter, T.nnet.softmax(normal_broadened))
...
#			self.trace = approx.sample(draws = num_draws)
			...
			approx     = pm.fit(n = 5 * 10**4, obj_optimizer = pm.adam(learning_rate = self.learning_rate))
			self.trace = approx.sample(draws = num_draws)

#			inference = pm.SVGD(n_particles = 500)
#			inference.fit(100, obj_optimizer = pm.adamax(learning_rate = 0.1))
#			approx = inference.approx
#			self.trace = approx.sample(draws = 100)
				
#			step_discrete = ElemwiseCategorical(self.out_cats_0)
#			self.trace = pm.sample(500, njobs = 2)

		print(self.network_output)
		print(self.param_index)
		print(self.network_output[:, self.param_index].shape)
#		print('_LOC', np.mean(self.trace['bnn_out'], axis = 0))
		print('LOC', np.mean(self.trace['loc'], axis = 0))
		print('PROBS', np.mean(self.trace['dirich_0'], axis = 0))
		print('PROBS_STD', np.std(self.trace['dirich_0'], axis = 0))


	#	import matplotlib.pyplot as plt 
	#	probs = self.trace['dirich_0']
	#	for dim in range(probs.shape[1]):
	#		for sample in range(probs.shape[2]):
	#			plt.plot(probs[:, dim, sample])
	#
	#	plt.plot(self.trace['dirich_0'])
	#	plt.show()
	#	plt.pause(30)
	#
		quit()
