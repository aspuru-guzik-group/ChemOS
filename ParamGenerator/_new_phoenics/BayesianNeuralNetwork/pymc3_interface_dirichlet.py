		...
		with pm.Model() as self.model:
			...
			for layer_index in range(self.num_layers):
				...
				if layer_index == 0:
					...
					print('SETTING LAST LAYER', layer_index)
			...
			self.out_floats = pm.Normal('out_floats', self.loc[:, self.floats], tau = self.tau[:, self.floats], observed = self.network_output[:, self.floats])


			# learn the integers
			self.out_ints = DiscreteLaplace('out_ints', loc = self.loc[:, self.ints], scale = self.scale[:, self.ints], observed = self.network_output[:, self.ints])


			# learn the categories
			dist_counter, cat_var_index = 0, 0
			self.alpha = pm.Deterministic('alpha', self._loc * self.tau)
			...
			for var_e_index, var_e_type in enumerate(self.var_e_types):
				if var_e_type == 'categorical' and self.var_e_begin[var_e_index] == var_e_index:
					...
					param_index = np.argwhere(self.var_p_names == var_e_name)[0]
