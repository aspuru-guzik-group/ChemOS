...
#	def _sample_posterior(self, num_samples = 200):
#		print('# LOG | compiling penalties ...')
#		self.generator.build_penalty()
#
#		uniform_samples = np.random.uniform(low = 0., high = 1., size = (num_samples, self.total_size))
#
#
...
#			print('# LOG | local optimization for batch index ', batch_index)
#			optimized_samples = []
#			for sample in uniform_samples:
#				result = minimize(self.generator.penalties[batch_index], sample, method = 'Powell', options = {'maxiter': 25})
#				rewards.append(np.exp( - self.temp * result.fun))
#				optimized_samples.append(result.x)
#			samples.append(optimized_samples)
#		self.proposed_samples = np.array(samples)
#		print(self.proposed_samples.shape)


#		print('# LOG | ...')
#		dot_prods = np.array([self.generator.get_dot_prods(probs) for probs in all_probs])
#
#		print('# LOG | collecting samples ...')
#		samples = []
#		for batch_index in range(self.num_batches):
#			print('# LOG | ... for ', batch_index)
...
#		indices = np.random.randint(low = 0, high = len(self.model_set), size = (num_samples))
		...
		all_probs = np.array([self.generator.get_probs(sample) for sample in proposals])
		denoms    = np.array([np.sum(probs) + 1. for probs in all_probs])
		dot_prods = np.array([self.generator.get_dot_prods(probs) for probs in all_probs])
		...
		for batch_index in range(self.num_batches):
			...
			to_optimize = proposals[sorted_reward_indices[-num_samples:]]
...
#		self.generator.build_penalty()
		...
		for batch_index in range(self.num_batches):
			...
			while len(batch_samples) < num_samples:
				...
				samples   = self.proposed_samples[batch_index][sorted_indices]
