...
#		with self.model:
#			theano.config.compute_test_value = 'ignore'
#			self.trace = pm.sample(self.num_samples)
...
#obj_optimizer
	...
	def build_penalty(self):
		...
		mus = self.trace['mean'][self.burnin::self.thinning]
		sds = self.trace['sd'][self.burnin::self.thinning]

		# mu shape: (# samples, # obs, out_dim)
...
#		theano.config.compute_test_value = 'ignore'
#		prob  = lambda x, mu, sd: T.prod(self._gauss(x, mu, sd), axis = 1)
#		probs = lambda x: T.mean( [prob(x, mus[i], sds[i]) for i in range(self.clean_samples)], axis = 0 )
#
#		x            = T.dvector('x')
#		lambda_value = T.scalar('lambda_value')
#		penalty      = (T.dot(self.losses, probs(x)) + lambda_value) / (T.sum(probs(x)) + 1.)
#		self.theano_penalty = theano.function(inputs = [x, lambda_value], outputs = penalty, name = 'penalty')
#		self.penalty = lambda x, index: self.theano_penalty(x, self.lambda_values[index])


		# no theano computation
		prob  = lambda x, mu, sd: np.prod(self._gauss(x, mu, sd), axis = 1)
		probs = lambda x: np.mean( [prob(x, mus[i], sds[i]) for i in range(self.clean_samples)], axis = 0 )
...
#		self.penalty = theano.function(inputs = [x, index], outputs = penalty, name = 'penalty')
#		self.penalty = lambda x, index: (np.dot(self.losses, probs(x)) + self.lambda_values[index]) / (np.sum(probs(x)) + 1.)

#		import matplotlib.pyplot as plt 
#		import seaborn as sns
#		domain = np.linspace(0, 1, 100)
#		for lambda_value_index, lambda_value in enumerate(self.lambda_values):
#			values = []
#			for x in domain:
#				values.append(self.penalty(x, lambda_value_index))
#		plt.legend()
#		plt.show()
#		quit()
